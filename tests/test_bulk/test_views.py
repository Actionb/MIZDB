...
class TestBulkAusgabe(ViewTestCase, LoggingTestMixin):
    ...
    def test_post_has_changed_message(self):
        ...
        self.assertMessageSent(
            response.wsgi_request, "Angaben haben sich geändert. Bitte kontrolliere diese in der Vorschau."
        )
    ...
    def test_save_data(self):
        ...
        for pk in before_save_ids:
            with self.subTest():
                self.assertTrue(self.queryset.filter(pk=pk).exists(), msg="Test object was deleted unexpectedly.")
    ...
    def test_save_data_updated(self):
        ...
        self.assertLoggedChange(self.updated, change_message=[{"changed": {"fields": ["Jahrgang"]}}])
    ...
    def test_save_data_created(self):
        ...
        self.assertEqual(len(before_save_ids), 3)
        self.assertEqual(sorted(before_save_ids), sorted([self.updated.pk, self.multi1.pk, self.multi2.pk]))
        ...
        for n in [2, 3, 4]:
            with self.subTest(num=n):
                ...
                self.assertNotIn(
                    qs.first(),
                    preexisting,
                    msg="Preexisting object found in the collection that should only contain explicitly new instances.",
                )
        ...
        for instance, expected_num in zip(created, [2, 3, 4]):
            ...
            if expected_num == 4:
                ...
                self.assertEqual(instance.bestand_set.first().lagerort, self.lo)
    ...
    def test_javascript(self):
        ...
        with self.settings(DEBUG=True):
            ...
            self.assertGreater(js.index(select2), js.index(jquery), msg="select2 must be loaded after jQuery.")
            self.assertIn(jquery_init, js)
            self.assertGreater(
                js.index(jquery_init), js.index(select2), msg="select2 must be loaded before django's jquery_init."
            )
    ...
    def test_story(self):
        ...
        first_preview_initial = dict(first_preview_request.session.get("old_form_data", {}).items())
