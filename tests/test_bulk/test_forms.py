...
class TestBulkForm(MIZTestCase):
    ...
    def test_clean_populating_split_data(self):
        ...
        test_data = [("num", ["1", "2"]), ("years", ["2000", "2001"])]
...
class TestBulkFormAusgabe(DataTestCase):
    ...
    def test_row_data_prop(self):
        ...
        row_1.update({"num": "1", "ausgabe_lagerort": self.dublette, "instance": self.updated})
        ...
        row_5.update({"num": "4", "ausgabe_lagerort": self.dublette, "dupe_of": row_4})
        row_6 = row_template.copy()
        # Data for this row will find more than one instance;
        # expected to show up in 'multiples'.
        row_6.update({"num": "5", "multiples": self.model.objects.filter(pk__in=[self.multi1.pk, self.multi2.pk])})
