...
class TestUtils(MIZTestCase):
    def test_get_dbfield_from_path(self):
        test_data = [
            (Artikel, "seite__gt", (Artikel._meta.get_field("seite"), ["gt"])),
            (
                Artikel,
                "ausgabe__magazin__magazin_name__icontains",
                (Magazin._meta.get_field("magazin_name"), ["icontains"]),
            ),
            (Ausgabe, "e_datum__year__gte", (Ausgabe._meta.get_field("e_datum"), ["year", "gte"])),
        ]
    ...
    def test_strip_lookups_from_path(self):
        ...
        for lookups, path, expected in test_data:
            with self.subTest(lookup=lookups, path=path):
                ...
                self.assertNotIn(f"{LOOKUP_SEP}{lookups}", utils.strip_lookups_from_path(path, lookups))
