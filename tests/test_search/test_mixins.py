...
class TestSearchFormMixin(ViewTestCase):
    ...
    def test_has_search_form(self):
        ...
        """
        test_data = [(None, False), ({}, False), ({"fields": []}, False), ({"fields": ["name"]}, True)]
    ...
    def test_check_search_form_fields_unknown_fields(self):
        ...
        with mock.patch("dbentry.search.mixins.get_model_fields", new=get_model_fields_mock):
            ...
            self.assertEqual(errors[0].msg, "Ignored search form field: 'BeepBoop'. Band has no field named 'BeepBoop'")
    ...
    def test_check_search_form_fields_invalid_lookup(self):
        ...
        with mock.patch("dbentry.search.mixins.get_model_fields", new=get_model_fields_mock):
            ...
            self.assertIsInstance(errors[0], checks.Info)
            self.assertEqual(
                errors[0].msg, "Ignored search form field: 'band_name__beep'. Invalid lookup: beep for CharField."
            )
    ...
    def test_check_search_form_fields_missing_fields(self):
        ...
        self.assertIsInstance(errors[0], checks.Info)
        self.assertEqual(errors[0].msg, "Changelist search form is missing fields for relations:\n\t['genre']")
...
class TestAdminMixin(AdminTestCase):
    ...
    def test_lookup_not_allowed(self):
        ...
        with mock.patch.object(self.model_admin, "search_form_kwargs", {"fields": [field_path]}):
            ...
            self.assertFalse(
                self.model_admin.lookup_allowed(f"{field_path}__icontains", None),
                msg=(
                    f"Lookup 'icontains' for {field_path} is not registered on the "
    ...
    def test_response_post_save_preserves_multi_values(self):
        ...
        response = self.client.get(path=self.change_path.format(pk=obj.pk), data=request_data)
        redirect = self.model_admin._response_post_save(response.wsgi_request, obj)
        query_string = urlparse(redirect.url)[4]
        self.assertEqual(sorted(QueryDict(query_string).lists()), sorted(QueryDict("genre=1&genre=2").lists()))
    ...
    def test_get_search_form_class_custom_form_class(self):
        ...
        with mock.patch.multiple(view, searchform_kwargs={}, searchform_factory=factory_mock, create=True):
            ...
            self.assertEqual(
                ...
                MIZAdminSearchForm,
                msg=(
                    f"factory should be called with default form class {MIZAdminSearchForm!r} "
                    f"if no other form class is provided."
                ),
            )
            ...
            self.assertEqual(
                kwargs["form"],
                "SearchFormKwargsForm",
                msg="factory should be called with the form class provided in the ModelAdmin's search_form_kwargs.",
            )
            ...
            self.assertEqual(
                kwargs["form"],
                "CallKwargsForm",
                msg="factory should be called with the form class provided in the kwargs to get_search_form_class.",
            )
...
class TestChangelistSearchFormMixin(RequestTestCase):
    ...
    def test_get_filters_params(self):
        ...
        search form.
        """
        request = self.get_request(data={"schlagzeile": "Schlagzeile", "seite_0": "10", "seite_1": "20"})
        changelist = self.SearchFormChangelist(request)
        changelist.model_admin = ArtikelAdmin(Artikel, admin_site)
        self.assertEqual(changelist.get_filters_params({}), {"schlagzeile": "Schlagzeile", "seite__range": [10, 20]})
...
class TestSearchFormChangelist(AdminTestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.obj1, cls.obj2, cls.obj3 = cls.test_data = [
            make(cls.model, schlagzeile="Object1", seite="10", genre=[cls.genre1, cls.genre2]),
