...
class FieldGroupTest(MIZTestCase):
    ...
    def test_fields_with_values(self):
        form = self.TestForm(
            data={
                "name": "Bob",
                "nickname": "Bobby",  # this group's fields has values
                "is_musician": [],
                "is_scientist": False,  # this group's fields are 'empty'
            }
        )
        form.full_clean()
    ...
    def test_check(self):
        form = self.TestForm(data={"name": "Brian", "nickname": "May", "is_musician": True, "is_scientist": True})
        form.full_clean()
        name_group = FieldGroup(form=form, fields=["name", "nickname"], min_fields=1)
        job_group = FieldGroup(form=form, fields=["is_musician", "is_scientist"], max_fields=1)
...
class MinMaxRequiredFormMixinTest(MIZTestCase):
    ...
    def test_get_group_error_messages(self):
        ...
        group = FieldGroup(
            form, fields=["first_name", "last_name"], min_fields=1, max_fields=1, error_messages=error_messages
        )
...
class DynamicChoiceFormMixinTest(MIZTestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.band_choices = [
            ...
            (str(o.pk), str(o))
            for o in [cls.obj3, cls.obj2, cls.obj1]  # noqa
        ]
...
class TestDeleteDuplicatesMixin(MIZTestCase):
    ...
    def test_validate_unique(self):
        ...
        msg = "Neither of the formset's forms should raise a ValidationError when validating unique constraints."
        with self.assertNotRaises(ValidationError, msg=msg):
            original.validate_unique()
            duplicate.validate_unique()
        self.assertTrue(duplicate.cleaned_data["DELETE"], msg="Expected the duplicate form to be flagged for deletion.")
...
class TestDiscogsFormMixin(MIZTestCase):
    ...
    def test_clean_no_match_id_and_url(self):
        ...
        """
        form = self.TestForm(data={"release_id": "1234", "discogs_url": "http://www.discogs.com/release/3512181"})
        form.full_clean()
        self.assertIn(NON_FIELD_ERRORS, form.errors)
        expected_error_message = (
            "Die angegebene Release ID (1234) stimmt nicht mit der ID im Discogs Link überein (3512181)."
        )
