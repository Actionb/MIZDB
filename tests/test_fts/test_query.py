...
class TestFullTextSearch(DataTestCase):
    ...
    def setUpTestData(cls):
        ...
        # 'Control' object:
        cls.obj2 = make(
            cls.model, band_name="The Beatles", beschreibung="The Beatles sollten in keinen Suchergebnissen auftauchen."
        )
    ...
    def test_ordering_ranked(self):
        ...
        another = make(self.model, band_name="Toten Hosen", beschreibung="Sie gehen gerne zum Arzt.")
        self.assertQuerySetEqual([exact, startsw, self.obj1, another], self.model.objects.search("Ärzte", ranked=True))
    ...
    def test_ordering_not_ranked(self):
        ...
        self.assertQuerySetEqual(
            [self.obj1, other, another], self.queryset.search("Die Ärzte", ranked=False).order_by("-rank")
        )
    ...
    def test_search_autor(self):
        ...
        """
        obj = make(_models.Autor, person__vorname="Peter", person__nachname="Lustig", kuerzel="PL")
        names = ("Peter Lustig", "Lustig, Peter", "Peter (PL) Lustig", "Peter Lustig (PL)", "Lustig, Peter (PL)")
        for name in names:
            with self.subTest(name=name):
...
class TestTextSearchQuerySetMixin(TestCase):
    ...
    def test_get_search_query(self):
        ...
        """
        search_query = self.queryset._get_search_query("Ardal O'Hanlon ", config="simple_unaccent", search_type="plain")
    ...
    def test_get_search_query_config_not_simple(self):
        ...
        """
        search_query = self.queryset._get_search_query("Ardal O'Hanlon ", config="german_unaccent", search_type="plain")
        _config, value = search_query.get_source_expressions()
        self.assertEqual(value.value, "Ardal O'Hanlon ")
    ...
    def test_get_related_search_vectors(self):
        ...
        """
        self.assertEqual(self.queryset._get_related_search_vectors(), [("alias__fts", "simple_unaccent")])
