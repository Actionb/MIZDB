...
class TestMIZQuerySet(DataTestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.obj2 = make(cls.model, band_name="Testband2", bandalias__alias=["Coffee"], genre__genre=["Rock", "Jazz"])
        cls.obj3 = make(
            cls.model, band_name="Testband3", bandalias__alias=["Juice", "Water"], genre__genre=["Rock", "Jazz"]
        )
        super().setUpTestData()
    ...
    def test_values_dict(self):
        ...
        expected = [
            ...
            (
                self.obj2.pk,
                {"band_name": ("Testband2",), "genre__genre": ("Jazz", "Rock"), "bandalias__alias": ("Coffee",)},
            ),
            (
                self.obj3.pk,
                {"band_name": ("Testband3",), "genre__genre": ("Jazz", "Rock"), "bandalias__alias": ("Juice", "Water")},
            ),
        ]
            ...
            with self.subTest(obj_pk=obj_pk):
                ...
                for field_name, _values in expected_values.items():
                    with self.subTest(field_name=field_name, values=_values):
                        ...
                        self.assertEqual(sorted(value_dict[field_name]), sorted(_values))
    ...
    def test_values_dict_include_empty(self):
        ...
        self.assertEqual(
            queryset.values_dict(*self.fields, include_empty=False)[self.obj1.pk], {"band_name": ("Testband1",)}
        )
...
class TestAusgabeChronologicalOrder(DataTestCase):
    ...
    def setUpTestData(cls):
        def get_date(month, year):  # noqa
            return f"{year!s}-{month!s}-{random.randrange(1, 28)}"
        ...
        for jg, year in enumerate(range(1999, 2002), start=1):
            for i in range(1, 4):
                cls.e_datum.append(make(cls.model, magazin=cls.mag, e_datum=get_date(i, year), ausgabejahr__jahr=year))
                cls.num.append(make(cls.model, magazin=cls.mag, ausgabenum__num=i, ausgabejahr__jahr=year))
                cls.lnum.append(make(cls.model, magazin=cls.mag, ausgabelnum__lnum=i, ausgabejahr__jahr=year))
                cls.monat.append(
                    make(cls.model, magazin=cls.mag, ausgabemonat__monat__ordinal=i, ausgabejahr__jahr=year)
                )
                cls.jg.append(make(cls.model, magazin=cls.mag, ausgabenum__num=i, jahrgang=jg))
    ...
    def test_checks_for_pk_ordering(self):
        ...
        for ordering in [[], ["pk"], ["-pk"], ["id"], ["-id"], ["pk", "-pk"]]:
            with self.subTest(ordering=ordering):
                ...
                if not ordering:
                    self.assertEqual(
                        queryset.query.order_by[-1],
                        f"-{self.model._meta.pk.name}",
                        msg="If no ordering is specified, the last ordering " "entry should default to '-{pk_name}'.",
                    )
                else:
                    self.assertEqual(
                        queryset.query.order_by[-1], ordering[0], msg="Last ordering entry should be a primary key."
                    )
    ...
    def test_jahr_over_jahrgang(self):
        ...
        self.assertGreater(
            order_by.index("jahrgang"), order_by.index("jahr"), msg="'jahr' ordering expected before 'jahrgang'."
        )
    ...
    def test_jahrgang_over_jahr(self):
        ...
        self.assertGreater(
            order_by.index("jahr"), order_by.index("jahrgang"), msg="'jahrgang' ordering expected before 'jahr'."
        )
    ...
    def test_table_join_duplicates(self):
        ...
        e = make(
            ...
            ausgabemonat__monat__ordinal=[1, 2, 3],
            ausgabenum__num=[1, 2, 3],
        )
        ordering = self.model.objects.filter(pk__in=[a.pk, b.pk, c.pk, d.pk, e.pk]).chronological_order().query.order_by
    ...
    def test_count_ordering_field_only_once_per_row(self):
        ...
        b = make(self.model, magazin=self.mag, e_datum="2023-04-16", ausgabelnum__lnum=[1, 2, 3])
...
class TestAusgabeIncrementJahrgang(DataTestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.obj2 = make(
            ...
            ausgabejahr__jahr=[2000],
            e_datum="2000-05-01",
            ausgabemonat__monat__ordinal=[5],
            ausgabenum__num=[5],
        )
        # obj3: start_jg - 1
        ...
        cls.obj4 = make(
            ...
            e_datum="2000-12-31",
            ausgabemonat__monat__ordinal=[12, 1],
    ...
    def test_increment_no_start_values(self):
        ...
        self.queryset.increment_jahrgang(start_obj=self.obj1, start_jg=10)
        self.assertFalse(self.queryset.exclude(pk=self.obj1.pk).filter(jahrgang__isnull=False).exists())
        self.assertSequenceEqual(self.queryset.filter(pk=self.obj1.pk).values_list("jahrgang", flat=True), [10])
