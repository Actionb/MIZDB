...
class TestModelAusgabe(MIZTestCase):
    ...
    def test_get_name_sonderausgabe(self):
        ...
        name_data.update({"jahrgang": ("2",), "ausgabejahr__jahr": ("2020",), "ausgabemonat__monat__abk": ("Dez",)})
        self.assertEqual(self.model._get_name(**name_data), "Test-Info")
    ...
    def test_get_name_no_data(self):
        ...
        test_data = [({}, "No data for Ausgabe."), ({"beschreibung": ("Test-Info",)}, "Test-Info")]
        for name_data, expected in test_data:
            with self.subTest(name_data=name_data):
                name = self.model._get_name(**name_data)
                self.assertEqual(name, expected)
    ...
    def test_get_name_ausgaben_merkmal(self):
        ...
        name_data = {"magazin__ausgaben_merkmal": ("lnum",), "ausgabelnum__lnum": ("21",)}
        self.assertEqual(
            self.model._get_name(**name_data),
            "21",
            msg=(
                "get_name should just return the lnum if ausgaben_merkmal is set to lnum "
                "and neither jahr nor jahrgang are set."
            ),
        )
        # Merkmal attribute is not set:
        name_data = {"magazin__ausgaben_merkmal": ("num",), "beschreibung": ("Whoops!",)}
        self.assertEqual(
            self.model._get_name(**name_data),
            "Whoops!",
            msg="get_name should ignore ausgaben_merkmal if the attribute " "it is referring to is not set.",
        )
...
class TestModelArtikel(MIZTestCase):
    ...
    def test_str(self):
        ...
        self.assertEqual(obj.__str__(), "Dies ist eine Testzusammenfassung, die nicht besonders lang ist.")

    def test_meta_ordering(self):
        """Assert that the ordering includes magazin_name and Ausgabe._name."""
        self.assertEqual(
            self.model._meta.ordering, ["ausgabe__magazin__magazin_name", "ausgabe___name", "seite", "schlagzeile"]
        )
...
class TestModelBestand(MIZTestCase):
    ...
    def test_bestand_object(self):
        ...
        """
        test_data = [("ausgabe", _models.Ausgabe), ("audio", _models.Audio), ("brochure", _models.Katalog)]
        lagerort = make(_models.Lagerort)
        for field_name, expected_model in test_data:
            with self.subTest(field_name=field_name):
                obj = _models.Bestand.objects.create(**{"lagerort": lagerort, field_name: make(expected_model)})
...
class TestModelBaseBrochure(MIZTestCase):
    ...
    def test_str(self):
        obj = make(self.model, titel="Model Inheritance Is Funky", zusammenfassung="Alan please add a summary.")
...
class TestM2mAudioMusiker(MIZTestCase):
    ...
    def test_str(self):
        obj = make(
            self.model,
            musiker__kuenstler_name="Natalia Lafourcade",
