...
from dbentry.admin.autocomplete.widgets import (
    EXTRA_DATA_KEY,
    GenericURLWidgetMixin,
    MIZModelSelect2,
    MIZModelSelect2Multiple,
    MIZModelSelect2MultipleTabular,
    MIZModelSelect2Tabular,
    MIZWidgetMixin,
    RemoteModelWidgetWrapper,
    TabularResultsMixin,
    make_widget,
)
from tests.case import MIZTestCase
from tests.test_admin_autocomplete.models import Artikel, Ausgabe, Inherited
...
@override_settings(ROOT_URLCONF="tests.test_admin_autocomplete.urls")
class TestRemoteModelWidgetWrapper(MIZTestCase):
    ...
    def test_get_related_url(self):
        url = self.widget.get_related_url(("test_admin_autocomplete", "ausgabe"), "change", "__fk__")
        self.assertEqual(
            url,
            "/admin/test_admin_autocomplete/ausgabe/__fk__/change/",
        )

        url = self.widget.get_related_url(("test_admin_autocomplete", "ausgabe"), "add")
        self.assertEqual(url, "/admin/test_admin_autocomplete/ausgabe/add/")

        url = self.widget.get_related_url(("test_admin_autocomplete", "ausgabe"), "delete", "__fk__")
        self.assertEqual(url, "/admin/test_admin_autocomplete/ausgabe/__fk__/delete/")
    ...
    def test_get_context_can_change(self):
        with patch.object(self.widget, "can_change_related", True):
            ...
            self.assertEqual(
                context.get("change_related_template_url"), "/admin/test_admin_autocomplete/ausgabe/__fk__/change/"
            )
    ...
    def test_get_context_can_add(self):
        with patch.object(self.widget, "can_add_related", True):
            ...
            self.assertEqual(context.get("add_related_url"), "/admin/test_admin_autocomplete/ausgabe/add/")
    ...
    def test_get_context_can_delete(self):
        with patch.object(self.widget, "can_delete_related", True):
            ...
            self.assertEqual(
                context.get("delete_related_template_url"), "/admin/test_admin_autocomplete/ausgabe/__fk__/delete/"
            )
...
class TestMIZWidgetMixin(MIZTestCase):
    ...
    def test_init(self):
        ...
        self.assertEqual(self.Widget(model_name="genre", create_field="Test").create_field, "Test")
...
# make_widget return type hint is set to Widget, but some tests expect a
# MIZModelSelect2 instance. This Union satisfies both.
Widget = Union[forms.Widget, MIZModelSelect2]
...
class TestMakeWidget(MIZTestCase):
    ...
    def test_exception_on_no_model(self):
        ...
        self.assertEqual(cm.exception.args[0], "MIZModelSelect2 widget missing argument 'model_name'.")

    def test_assigns_create_field(self):
        widget: Widget = make_widget(model=Ausgabe)
    ...
    def test_forwarded(self):
        ...
        widget: Widget
        # Remember that the values for forward are wrapped in a forward.Field
        # object.
        ...
        widget = make_widget(model=Ausgabe, forward="magazin", attrs={"data-placeholder": "Test placeholder"})
        self.assertEqual(widget.forward[0].src, "magazin")
    ...
    def test_forwarded_empty_values(self):
        ...
        for val in [(), [], None, ""]:
            with self.subTest(value=val):
                widget: Widget = make_widget(model=Ausgabe, forward=val)
