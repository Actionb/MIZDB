...
from tests.test_admin_autocomplete.models import Band, Genre, Musiker
...
class ACViewTestCase(ViewTestCase):
    ...
    def get_view(
        self, request=None, args=None, kwargs=None, model=None, create_field=None, forwarded=None, q="", **initkwargs
    ):
        initkwargs = {
    ...
    def test_get_create_option(self):
        ...
        with patch.object(view, "display_create_option") as display_option_mock:
            with patch.object(view, "build_create_option") as build_option_mock:
                ...
                self.assertEqual(view.get_create_option(context={}, q=q), "This would be the create option!")
...
class TestACTabular(ACViewTestCase):
    ...
    def test_render_to_response_grouped_data(self):
        ...
        context = {"object_list": [self.obj], "page_obj": view.paginate_queryset(self.model.objects.all(), 10)[1]}
    ...
    def test_render_to_response_not_first_page(self):
        ...
        context = {"object_list": [self.obj], "page_obj": view.paginate_queryset(self.model.objects.all(), 1)[1]}
...
class TestACBand(ACViewTestCase):
    ...
    def test_result_ordering(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual([str(self.exact.pk), str(self.startsw.pk), str(self.contains.pk)], get_result_ids(response))
    ...
    def test_filter_with_forwarded_values(self):
        ...
        response = self.get_response(
            ...
            self.path,
            data={"text": "foo", "forward": f'{{"genre": "{self.genre.pk}"}}'},
        )
...
class TestACAusgabe(ACViewTestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.obj_num = make(cls.model, magazin=mag, ausgabejahr__jahr=2020, ausgabenum__num=10)
        cls.obj_lnum = make(cls.model, magazin=mag, ausgabejahr__jahr=2020, ausgabelnum__lnum=11)
        cls.obj_monat = make(cls.model, magazin=mag, ausgabejahr__jahr=2020, ausgabemonat__monat__monat="Januar")
        cls.obj_sonder = make(cls.model, magazin=mag, sonderausgabe=True, beschreibung="Special Edition")
        ...
        cls.test_data = [cls.obj_num, cls.obj_lnum, cls.obj_monat, cls.obj_sonder, cls.obj_jahrg]  # noqa
...
class TestACAutor(ACViewTestCase):
    ...
    def test(self):
        """Assert that an autocomplete request returns the expected results."""
        obj = make(self.model, kuerzel="AT", person__vorname="Alice", person__nachname="Testman")
...
class TestACSpielort(ACViewTestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.obj = make(
            cls.model,
            name="Freizeitzentrum West",
            spielortalias__alias="FZW",
            ort=ort,
            beschreibung="Ort für Konzerte",
            bemerkungen="Braucht mehr Info!",
        )
        super().setUpTestData()
...
class TestACVeranstaltung(RequestTestCase):
    ...
    def test(self):
        """Assert that an autocomplete request returns the expected results."""
        search_terms = [self.obj.pk, "Woodstock", "1969", "Rock Festival", "Yasgur", "Bethel", "Love", "Info"]
        for search_term in search_terms:
            with self.subTest(search_term=search_term):
                response = self.get_response(self.path, data={"q": search_term})
                self.assertIn(str(self.obj.pk), get_result_ids(response))
...
class TestGND(ViewTestCase):
    ...
    def test(self, query_mock):
        ...
        self.assertEqual(len(results), 1)
        expected = [
            {"id": "134485904", "text": "Plant, Robert (134485904)", "selected_text": "Plant, Robert (134485904)"}
        ]
    ...
    def test_get_queryset_page_number(self, mocked_query_func):
        ...
        start_record_msg = "Expected query func to be called with a 'startRecord' kwarg."
        view_initkwargs = {
            "q": "Beep",
            "page_kwarg": "page",
            "paginate_by": 10,
        ...
        self.assertIn("startRecord", kwargs, msg=start_record_msg)
    ...
    def test_get_queryset_paginate_by(self, mocked_query_func):
        ...
        self.assertIn("startRecord", kwargs, msg="Expected query func to be called with a 'startRecord' kwarg.")
        self.assertEqual(kwargs["startRecord"], ["11"])
    ...
    def test_get_queryset_maximum_records(self, mocked_query_func):
        ...
        view.get_queryset()
        _args, kwargs = mocked_query_func.call_args
        self.assertIn("maximumRecords", kwargs, msg="Expected query func to be called with a 'maximumRecords' kwarg.")
...
class TestGNDPaginator(MIZTestCase):
    ...
    def test_page_does_not_slice_object_list(self):
        ...
        """
        # Mock object isn't subscript-able; trying to slice it would raise a TypeError.
        paginator = views.GNDPaginator(object_list=Mock(), per_page=1, total_count=1, allow_empty_first_page=True)
...
class TestContentTypeAutocompleteView(ACViewTestCase):
    ...
    def test_get_queryset(self):
        ...
        self.assertQuerySetEqual(
            view.get_queryset(), ContentType.objects.filter(app_label=opts.app_label, model=opts.model_name)
        )
