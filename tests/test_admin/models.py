...
class Band(BaseModel):
    ...
    def get_overview_annotations():
        return {
            "alias_list": ArrayAgg("bandalias__alias", distinct=True, ordering="bandalias__alias"),
        }
...
class BandAlias(models.Model):
    ...
    band = models.ForeignKey("test_admin.Band", on_delete=models.CASCADE)


# Model for BaseM2MModel tests:
class MusikerAudioM2M(BaseM2MModel):
    audio = models.ForeignKey("test_admin.Audio", on_delete=models.CASCADE)
    musiker = models.ForeignKey("test_admin.Musiker", on_delete=models.CASCADE)
...
class Audio(BaseModel):
    ...
    reihe = models.ForeignKey("test_admin.AudioReihe", on_delete=models.SET_NULL, blank=True, null=True)

    musiker = models.ManyToManyField("test_admin.Musiker", through=MusikerAudioM2M)
    band = models.ManyToManyField("test_admin.Band")

    veranstaltung = models.ManyToManyField("test_admin.Veranstaltung")
...
class Veranstaltung(models.Model):
    ...
    reihe = models.ForeignKey("test_admin.VeranstaltungsReihe", on_delete=models.SET_NULL, blank=True, null=True)

    musiker = models.ManyToManyField("test_admin.Musiker")
    band = models.ManyToManyField("test_admin.Band")
...
class Bestand(models.Model):
    lagerort = models.ForeignKey("test_admin.Lagerort", models.PROTECT)

    audio = models.ForeignKey("test_admin.Audio", on_delete=models.CASCADE, blank=True, null=True)
