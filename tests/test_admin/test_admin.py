...
class AdminTestMethodsMixin(object):
    ...
    # Fields to be excluded from the change view form:
    exclude_expected = ()
    # Fields expected to be on the change view form:
    fields_expected = ()
    ...
    def get_annotated_model_obj(self: Union["AdminTestMethodsMixin", "AdminTestCase"], obj):
        ...
        return self.queryset.filter(pk=obj.pk).annotate(**self.model.get_overview_annotations()).get()
    ...
    def test_get_exclude(self: Union["AdminTestMethodsMixin", "AdminTestCase"]):
        ...
        self.assertCountEqual(self.model_admin.get_exclude(self.get_request()), self.exclude_expected)
    ...
    def test_get_fields(self: Union["AdminTestMethodsMixin", "AdminTestCase"]):
        ...
        self.assertSequenceEqual(self.model_admin.get_fields(self.get_request()), self.fields_expected)
...
class TestArtikelAdmin(AdminTestMethodsMixin, AdminTestCase):
    ...
    def test_zusammenfassung_string(self):
        ...
        self.obj1.zusammenfassung = (
            "Dies ist eine Testzusammenfassung, die nicht besonders inhaltsvoll " "ist, dafür aber doch recht lang ist."
        )
        self.assertEqual(
...
class TestAudioAdmin(AdminTestMethodsMixin, AdminTestCase):
    ...
    @classmethod
    def setUpTestData(cls):
        cls.obj1 = make(cls.model, band__band_name="Testband", musiker__kuenstler_name="Alice Tester")
        super().setUpTestData()
...
class TestAusgabenAdmin(AdminTestMethodsMixin, AdminTestCase):
    ...
    def test_change_status_logentry_error(self, log_change_mock):
        ...
        self.assertEqual(message, "Fehler beim Erstellen der LogEntry Objekte: \nValueError: This is a test exception.")

    def test_has_moveto_brochure_permission(self):
        """Assert that the moveto_brochure action requires certain permissions."""
        msg_template = "Action 'moveto_brochure' should not be available to " "users that miss the '%s' permission."
        delete_codename = get_permission_codename("delete", _models.Ausgabe._meta)
        delete_permission = Permission.objects.get(
            codename=delete_codename, content_type=ContentType.objects.get_for_model(_models.Ausgabe)
        )
        add_codename = get_permission_codename("add", _models.BaseBrochure._meta)
        add_permission = Permission.objects.get(
            codename=add_codename, content_type=ContentType.objects.get_for_model(_models.BaseBrochure)
        )
        ...
        self.assertFalse(self.model_admin.has_moveto_brochure_permission(request), msg=msg_template % add_codename)

        # add only: not allowed
        user.user_permissions.set([add_permission])
        request = self.get_request(user=User.objects.get(pk=user.id))
        self.assertFalse(self.model_admin.has_moveto_brochure_permission(request), msg=msg_template % delete_codename)
    ...
    def test_actions_staff_user(self):
        ...
        for perm_name in ("change", "delete", "merge"):
            ...
            self.staff_user.user_permissions.add(Permission.objects.get(codename=codename, content_type=ct))
    ...
    def test_brochure_changelist_link(self):
        ...
        labels = {"brochure": "Broschüren (1)", "kalender": "Programmhefte (1)", "katalog": "Warenkataloge (1)"}
...
class TestBestandAdmin(AdminTestMethodsMixin, AdminTestCase):
    ...
    def test_cache_bestand_data(self):
        ...
        self.model_admin.cache_bestand_data(
            self.get_request(), result_list=self.queryset, bestand_fields=[self.model._meta.get_field("audio")]
        )
...
class TestBaseBrochureAdmin(AdminTestCase):
    ...
    def test_jahr_list(self):
        obj = self.queryset.filter(pk=self.obj1.pk).annotate(**self.model.get_overview_annotations()).get()
...
class TestBuchAdmin(AdminTestMethodsMixin, AdminTestCase):
    ...
    def test_autoren_string(self):
        obj = self.get_annotated_model_obj(self.obj1)
        self.assertEqual(self.model_admin.autoren_string(obj), "Alice Testman (AT), Bob Mantest (BM)")
...
class TestMagazinAdmin(AdminTestMethodsMixin, AdminTestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.obj1 = make(cls.model, ausgabe__extra=1, orte=[ort1, ort2], beschreibung=cls.beschreibung)  # noqa
        super().setUpTestData()
    ...
    def test_short_beschreibung(self):
        self.assertGreater(len(self.beschreibung), len(self.model_admin.short_beschreibung(self.obj1)))
...
class TestMusikerAdmin(AdminTestMethodsMixin, AdminTestCase):
    ...
    def setUpTestData(cls):
        cls.obj1 = make(cls.model)
        cls.obj2 = make(
            cls.model, band__band_name=["Testband1", "Testband2"], genre__genre=["Testgenre1", "Testgenre2"]
        )
        super().setUpTestData()
...
class TestPersonAdmin(AdminTestMethodsMixin, AdminTestCase):
    ...
    fields_expected = ["vorname", "nachname", "gnd_id", "gnd_name", "dnb_url", "beschreibung", "bemerkungen"]
    ...
    def test_is_musiker(self):
        obj1, obj2 = (
            self.queryset.annotate(**self.model.get_overview_annotations())
...
class TestPlakatAdmin(AdminTestMethodsMixin, AdminTestCase):
    ...
    fields_expected = ["titel", "plakat_id", "size", "datum", "reihe", "copy_related", "beschreibung", "bemerkungen"]
    exclude_expected = ["genre", "schlagwort", "person", "band", "musiker", "ort", "spielort", "veranstaltung"]
    ...
    def test_veranstaltung_string(self):
        obj = self.get_annotated_model_obj(self.obj1)
        self.assertEqual(self.model_admin.veranstaltung_string(obj), "Glastonbury 2004, Woodstock 1969")
    ...
    def test_save_related_calls_copy_related(self):
        ...
        with patch("dbentry.admin.admin.super"):
            with patch.object(self.model_admin, "_copy_related") as copy_related_mock:
                self.model_admin.save_related(request=None, form=form_mock, formsets=None, change=True)
...
class TestSchlagwortAdmin(AdminTestMethodsMixin, AdminTestCase):
    ...
    @classmethod
    def setUpTestData(cls):
        cls.obj1 = make(cls.model, schlagwort="Sub Schlagwort", schlagwortalias__alias=["Alias1", "Alias2"])
        super().setUpTestData()
...
class TestVeranstaltungAdmin(AdminTestMethodsMixin, AdminTestCase):
    ...
    def setUpTestData(cls):
        cls.obj1 = make(
            cls.model, datum="2022-10-17", band__band_name="Led Zeppelin", musiker__kuenstler_name="Robert Plant"
        )
        super().setUpTestData()
...
class TestVideoAdmin(AdminTestMethodsMixin, AdminTestCase):
    ...
    exclude_expected = ["band", "genre", "musiker", "person", "schlagwort", "ort", "spielort", "veranstaltung"]

    @classmethod
    def setUpTestData(cls):
        cls.obj1 = make(cls.model, band__band_name="Led Zeppelin", musiker__kuenstler_name="Robert Plant")
        super().setUpTestData()
...
class TestChangelistAnnotations(AdminTestCase):
    ...
    @classmethod
    def setUpTestData(cls):
        cls.obj = make(cls.model, status=_models.Ausgabe.Status.UNBEARBEITET, ausgabejahr__jahr=2022, ausgabenum__num=2)
        super().setUpTestData()
...
class TestAuthAdminMixin(TestCase):
    ...
    def test_formfield_for_manytomany(self, mocked_super):
        ...
        mocked_super.return_value = Mock(formfield_for_manytomany=Mock(return_value=mocked_formfield))
        formfield = _admin.AuthAdminMixin().formfield_for_manytomany(None)  # noqa
...
class TestMIZLogEntryAdmin(AdminTestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.obj = LogEntry.objects.log_action(
            ...
            action_flag=admin.models.CHANGE,
            change_message=[{"changed": {"fields": ["username"], "name": "User", "object": "Superuser"}}],
        )

    def test_object(self):
        """Assert that the object method returns a link to the given object."""
        opts = User._meta
        url = reverse(f"{self.admin_site.name}:{opts.app_label}_{opts.model_name}_change", args=[self.super_user.pk])
    ...
    def test_change_message_verbose(self):
        ...
        with translation_override(language=None):
            self.assertEqual(
                self.model_admin.change_message_verbose(self.obj), "Changed username for User “Superuser”."
            )
