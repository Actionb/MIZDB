...
class TestFuncs(unittest.TestCase):
    ...
    def test_install_system_packages(self, run):
        ...
        self.assertTrue(
            any(
                call.args[0].startswith("sudo apt install")
    ...
    def test_checks_postgres_server_version_unexpected_string(self, run, confirm):
        # check_postgres_server should catch exceptions raised from an
        # unexpected string

        # "X" would raise an IndexError, and "dont make this crash" a ValueError
        for query_string in ("X", "dont make this crash"):
    ...
    def test_create_venv_exists(self, run, confirm):
        # create_venv should return early if the venv already exists and the
    ...
    def test_create_config(self, run):
        ...
        # TODO: use tmpfiles instead of mocking open/write?
        with mock.patch('install.open') as mocked_open:
            mocked_write = mock.Mock()
            mocked_open.return_value.__enter__.return_value = mock.Mock(write=mocked_write)
            run.return_value = mock.Mock(
                returncode=0,
                stdout=b"aksjdzeuzrhsd"
            )
            ...
            config = mocked_write.call_args[0][0]
            self.assertIn("SECRET_KEY: 'aksjdzeuzrhsd'", config)
    ...
    def test_install_mod_wsgi(self, run):
        venv_directory = "~/somewhere/overtherainbow"
        run.return_value = mock.Mock(returncode=0)
        # TODO: use tmpfiles instead of mocking open/write?
        with mock.patch('install.open') as mocked_open:
            mocked_write = mock.Mock()
...
class TestInstall(unittest.TestCase):
    ...
    def patch(self, *exclude):
        ...
        for attr in to_patch:
            if attr in exclude:
                continue
            patch = mock.patch(attr, new=mock.Mock())
            patches.append(patch)
            setattr(self, attr.rsplit('.', 1)[-1], patch.new)
            patch.start()
        yield
