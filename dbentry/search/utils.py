...
def get_dbfield_from_path(model: Union[Model, Type[Model]], field_path: str) -> Tuple[Field, List[str]]:
    """
...
def strip_lookups_from_path(path: str, lookups: List[str]) -> str:
    ...
    return LOOKUP_SEP.join((path.split(LOOKUP_SEP)[0], *filter(filter_func, path.split(LOOKUP_SEP)[1:])))
