...
class RangeWidget(forms.MultiWidget):
    ...
    class Media:
        css = {"all": ("mizdb/css/rangewidget.css",)}
...
class SearchForm(forms.Form):
    ...
    Attributes:
        - ``lookups``: a mapping of {formfield_name: field lookups} that
          contains valid lookups for a given formfield. This mapping is filled
          by the search form factory. To add lookups for declared fields (i.e.
          fields not generated by the factory), overwrite init and update
          `self.lookups` after calling super.
        ...
        for field_name, value in self.cleaned_data.items():
            ...
            if isinstance(formfield, RangeFormField):
                ...
                if start_empty and end_empty:
                    ...
                    param_key = "".join([field_name, LOOKUP_SEP, self.range_upper_bound.lookup_name])
                    param_value = end
            elif (
                ...
                continue
            elif "in" in self.lookups.get(field_name, []) and isinstance(value, QuerySet):
                # django admin prepare_lookup_value expects a single string
                # of comma separated values for this lookup.
                param_value = ",".join(str(pk) for pk in value.values_list("pk", flat=True).order_by("pk"))
    ...
    def clean_id__in(self) -> str:
        ...
        return "".join(i for i in self.cleaned_data.get("id__in", "") if i.isnumeric() or i == ",")
...
class SearchFormFactory:
    ...
    def get_formfield(
        self, db_field: Field, formfield_class: Optional[forms.Field] = None, **kwargs: Any
    ) -> forms.Field:
        """
    ...
    def get_search_form(
        ...
        **kwargs: Any,
        ...
        for path in fields or []:
            ...
            if localized_fields == forms.models.ALL_FIELDS or (localized_fields and path in localized_fields):
