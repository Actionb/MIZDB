...
class TestCreator(DataTestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.alice_autor = make(autor, person=cls.alice, kuerzel = 'AT')
        cls.bob1_autor = make(autor, person=cls.bob1, kuerzel = 'bob1')
        cls.bob2_autor = make(autor, person=cls.bob2, kuerzel = 'bob2')
        
        cls.test_data = [cls.alice, cls.alice_autor, cls.bob1, cls.bob2, cls.bob1_autor, cls.bob2_autor]
    ...
    def test_create(self):
        ...
        self.assertIsNotNone(created['instance'], msg = "create(preview = False): returned dict must contain a model instance")
        self.assertIsNotNone(created['instance'].pk, msg = "create(preview = False): must return a saved instance")
    ...
    def test_create_creator_func_is_none(self):
        ...
        self.assertFalse(
            created, 
            msg = "create(preview = True: should return an empty dictionary if no creator function is set"
        )
    ...
    def test_create_reraises_or_failed_object(self):
        ...
        with self.assertRaises(MultipleObjectsReturnedException, msg = "create(preview = False): MultipleObjectsReturnedException exceptions should bubble up"):
            creator.create('Alice Testman', preview = False)
            
        creator.raise_exceptions = False
        with self.assertNotRaises(MultipleObjectsReturnedException, msg = "create(preview = False): No exceptions should bubble up if raise_exceptions == False"):
            created = creator.create('Alice Testman', False)
            self.assertIn('instance', created)
            self.assertIsInstance(created['instance'], FailedObject, msg = "create(preview = False): Error handling should have returned a FailedObject instance")
        
    def test_create_preview(self):
        creator = Creator(model = person, raise_exceptions = False)
        self.assertEqual(
            creator.create('Bob Testman', preview = True), {}, 
            msg = "create(preview = False): create preview should return an empty dict if multiple records in database fit the given parameters"
        ) 
    ...
    def test_get_model_instance(self):
        ...
        with self.assertRaises(MultipleObjectsReturnedException):
            creator._get_model_instance(person, **bob_data)
    ...
    def test_create_person(self):
        ...
        self.assertIn('Vorname', created, msg = "create_person(preview = True): a Vorname key is expected")
        self.assertEqual(created['Vorname'], 'Bob')
        self.assertIn('Nachname', created, msg = "create_person(preview = True): a Nachname key is expected")
        self.assertEqual(created['Nachname'], 'Tester')
        self.assertIn('instance', created)
        self.assertIsNotNone(created['instance'], msg = "create_person(preview = True): must return an instance")
        self.assertIsNone(created['instance'].pk, msg = "create_person(preview = True): must return an unsaved instance")
        
        created = creator.create_person('Bob Tester', preview = False)
        self.assertIn('Vorname', created, msg = "create_person(preview = False): a Vorname key is expected")
        self.assertEqual(created['Vorname'], 'Bob')
        self.assertIn('Nachname', created, msg = "create_person(preview = False): a Nachname key is expected")
        self.assertEqual(created['Nachname'], 'Tester')
        self.assertIn('instance', created)
        self.assertIsNotNone(created['instance'], msg = "create_person(preview = False): a person instance is expected")
        self.assertIsNotNone(created['instance'].pk, "create_person(preview = False): must return a saved instance")
    ...
    def test_create_autor(self):
        ...
        self.assertIn('Person', created, msg = "create_autor(preview = True): a Person key is expected")
        ...
        self.assertIsNotNone(created['Person']['instance'], msg = "create_autor(preview = True): must return a person instance")
        self.assertIsNone(created['Person']['instance'].pk, msg = "create_autor(preview = True): must return an unsaved instance")
        self.assertIn('Kürzel', created, msg = "create_autor(preview = True): a Kürzel key is expected")
        
        created = creator.create_autor('Bob Tester(BT)', preview = False)
        self.assertIn('Person', created, msg = "create_autor(preview = False): a Person key is expected")
        self.assertIn('Vorname', created['Person'])
        self.assertEqual(created['Person']['Vorname'], 'Bob')
        self.assertIn('Nachname', created['Person'])
        self.assertEqual(created['Person']['Nachname'], 'Tester')
        self.assertIn('instance', created['Person'])
        self.assertIsNotNone(created['Person']['instance'], msg = "create_autor(preview = False): must return a person instance")
        self.assertIn('Kürzel', created, msg = "create_autor(preview = False): a Kürzel key is expected")

        self.assertEqual(created['Kürzel'], 'BT')
        self.assertIn('instance', created)
        self.assertIsNotNone(created['instance'], msg = "create_autor(preview = False): must return an autor instance")
        self.assertIsNotNone(created['instance'].pk, "create_autor(preview = False): must return a saved instance")
        ...
        creator._get_model_instance.assert_called_with(autor, person = self.alice, kuerzel = 'NI')
