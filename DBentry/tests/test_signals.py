...
class TestSignalsAusgabe(DataTestCase):
    ...
    def test_signal_recieved_from_ausgabe_jahr_post_saved(self):
        self.assertFalse(self.qs_obj1.values_list('_changed_flag',flat=True).first())
        ausgabe_jahr.objects.create(ausgabe=self.obj1, jahr=1)
        self.assertTrue(self.qs_obj1.values_list('_changed_flag',flat=True).first())
    ...
    def test_signal_recieved_from_ausgabe_jahr_post_delete(self):
        self.assertFalse(self.qs_obj2.values_list('_changed_flag',flat=True).first())
        ausgabe_jahr.objects.filter(ausgabe=self.obj2).delete()
        self.assertTrue(self.qs_obj2.values_list('_changed_flag',flat=True).first())
