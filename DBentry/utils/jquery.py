...
def ensure_jquery(media):
    """
    Ensure that jquery is loaded first and in the right order
    (jquery.js -> jquery.init.js).
    Can be called with a media object or can be used as a method decorator.
    """
    def _ensure_jquery(_media):
        from django.conf import settings
        jquery_base = 'admin/js/vendor/jquery/jquery%s.js' % (
            '' if settings.DEBUG else '.min'
        )
        jquery_init = 'admin/js/jquery.init.js'
        if not _media._js_lists:
            # Media had no javascripts!
            _media._js_lists.append([jquery_base, jquery_init])
            return _media
        # Insert jquery_base at the beginning and jquery_init right after.
        # Remove pre-existing entries if necessary.
        # Cast the iterable js_list into a list for easier operations;
        # if it's a tuple, change it back to that after we're done.
    ...
    if isinstance(media, Media):
        ...
        # Decorating the media property;
        # decorator will try media.__get__(instance).
        return property(wrapper('__get__'))
    else:
        # Decorating the media func;
        # decorator will try media.__call__(instance).
