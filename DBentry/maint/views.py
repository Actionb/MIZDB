...
from DBentry.maint.forms import (
    DuplicateFieldsSelectForm, duplicatefieldsform_factory, ModelSelectForm
)
...
class MaintView(MIZAdminMixin, views.generic.TemplateView):
    ...
    def show_on_index_page(request):
        # Only show a link on the index page for superusers.
...
class DuplicateObjectsView(MaintView):
    ...
    def dispatch(self, request, *args, **kwargs):
        self.model = utils.get_model_from_string(kwargs.get('model_name'))
    ...
    def build_duplicate_items_context(self, form = None, dupe_fields = None):
        """
        Return a list of headers and a list of 2-tuples of:
        ...
        for instances, values in duplicates:
            dupe_item = [
                (instance, utils.get_obj_link(instance, self.request.user, include_name = False), [values[f] for f in dupe_fields]) #FIXME: values is a tuple!
